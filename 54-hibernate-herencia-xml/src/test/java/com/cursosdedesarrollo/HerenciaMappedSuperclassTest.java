...
/**
 * @MappedSuperclass equivalente en HBM: herencia de MAPEO sin entidad base.
 *
 * EntidadAuditable NO es una entidad Hibernate:
 *   - No existe tabla "entidades_auditables".
 *   - No se puede hacer "from EntidadAuditable" en HQL.
 *   - Sus campos (creado_en, modificado_en) se REPITEN manualmente en
 *     Factura.hbm.xml y Pedido.hbm.xml (a diferencia del módulo de
 *     anotaciones donde @MappedSuperclass los propaga automáticamente).
 *
 * Esta es la diferencia clave entre el enfoque XML y el de anotaciones
 * para este patrón: en XML se requiere duplicación explícita en el mapeo.
 *
 * DDL resultante:
